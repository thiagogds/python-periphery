...
class Serial(object):
    ...
    def read(self, length, timeout=None):
        ...
        return when at least VMIN bytes are read. For a blocking read with both
        VMIN and VTIME settings configured, `read()` will return when at least
        VMIN bytes are read or when the VTIME interbyte timeout expires after
        the last byte read. In both cases, `read()` may return less than the
    ...
    """Termios setting for minimum number of bytes returned from a blocking
    read. Can be between 0 and 255.
    ...
    return when at least VMIN bytes are read or when the VTIME timeout expires
    after the last byte read. See the `termios` man page for more information.
    ...
    """Termios setting for timeout in seconds of a blocking read. Can be
    between 0 to 25.5 seconds with a resolution of 0.1 seconds.
